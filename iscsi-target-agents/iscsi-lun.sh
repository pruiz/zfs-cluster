...
# for a default, use the first 16 characters of  $OCF_RESKEY_name.
#OCF_RESKEY_scsi_id_default="${OCF_RESOURCE_INSTANCE:0:16}"
OCF_RESKEY_scsi_id_default="${OCF_RESKEY_name}"
: ${OCF_RESKEY_scsi_id:=${OCF_RESKEY_scsi_id_default}}
# To have a reasonably unique default SCSI SN, use the first 8 bytes
# of an MD5 hash of of $OCF_RESKEY_name.
#sn=`echo -n "${OCF_RESOURCE_INSTANCE}" | md5sum`
sn=`echo -n "${OCF_RESKEY_scsi_id}" | md5sum`
...
<resource-agent name="iscsi-lun" version="0.9">
<version>0.9</version>
...
<parameters>
<parameter name="name" required="1" unique="1" primary="1">
<longdesc lang="en">
iSCSI LUN Resource unique name.
</longdesc>
<shortdesc lang="en">iSCSI LUN unique name</shortdesc>
<content type="string" />
</parameter>
...
<parameter name="bstype" required="0" unique="0">
<longdesc lang="en">
Backing Store type to use for this LU. (stgt-specific).
</longdesc>
<shortdesc lang="en">Backing Store type</shortdesc>
<content type="string" />
</parameter>
...
iSCSILogicalUnit_start() {
	...
	tgt)
	    ...
	    params="${params} ${OCF_RESKEY_additional_parameters}"
	    if [ -n "$OCF_RESKEY_bstype" ]; then
		bstype="--bstype=${OCF_RESKEY_bstype}"
	    fi
	    ocf_run tgtadm --lld iscsi --op new --mode logicalunit \
		--tid=${TID} \
		--lun=${OCF_RESKEY_lun} \
	    	--backing-store ${OCF_RESKEY_path} ${bstype} || exit $OCF_ERR_GENERIC
	    if [ -z "$params" ]; then
		return $OCF_SUCCESS
	    else
	...
	lio)
	    ...
	    ocf_run lio_node --addlun=${OCF_RESKEY_iqn} 1 ${OCF_RESKEY_lun} \
...
iSCSILogicalUnit_stop() {
	...
	case $OCF_RESKEY_implementation in
		...
		;;
	    lio)
		ocf_run lio_node --dellun=${OCF_RESKEY_iqn} 1 ${OCF_RESKEY_lun} || exit $OCF_ERR_GENERIC
...
iSCSILogicalUnit_monitor() {
	...
	tgt)
	    ...
	    ;;
	lio)
	    configfs_path="/sys/kernel/config/target/iscsi/${OCF_RESKEY_iqn}/tpgt_1/lun/lun_${OCF_RESKEY_lun}/${OCF_RESOURCE_INSTANCE}/udev_path"
...
iSCSILogicalUnit_validate() {
    ...
    for var in name implementation iqn lun path; do
